...
from hparams import hparams, DEVICE_DICT, CITY_DICT, SCENE_DICT, M_SCENE_DICT
...
def data_list_to_batch(data_list: [DataWithLabel]):
    ...
    for i, data_with_label in enumerate(data_list):
        ...
        scene_label[i] = M_SCENE_DICT[scene]
...
class CRNN_Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc_3 = nn.Linear(16 * output_T, len(M_SCENE_DICT))
...
if __name__ == '__main__':
    ...
    run_name = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + '_task_B'
